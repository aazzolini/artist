...
def generate_letter_svg(
    ...
    safe_font = (
        font_family
        if font_family in ["Arial", "Helvetica", "sans-serif"]
        else "sans-serif"
    )
    ...
    if rotation == 0:
        ...
        g = dwg.g(
            transform="translate({},{}) rotate({})".format(center_x, text_y, rotation)
        )
        text = dwg.text(
            letter,
            insert=(0, 0),
...
if __name__ == "__main__":
    ...
    save_interval = (
        None  #  100  # Save every 100th letter (optional, set to None to never save)
    )
    ...
    for svg, img_array, metadata in generate_random_letters():
        ...
        print(
            f"[{i}] Generated '{letter}' - Font: {metadata['font_family']}, "
        ...
        if i < 5:  # Do this for first 5 letters as example
            ...
            letter_canvas = GPUCanvas.from_array(
                img_array[:, :, :3]
            )  # Take RGB channels
