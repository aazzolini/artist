...
class GPUCanvas:
    ...
    def __init__(self, width, height, device=None):
        ...
        self.canvas = torch.ones((self.height, self.width, 3), dtype=torch.uint8, device=self.device) * 255
    ...
    def from_array(cls, img_array, device=None):
        ...
        if img_array.shape[2] == 4:
            ...
            raise ValueError(f"Expected 3 or 4 channels (RGB/RGBA), got {img_array.shape[2]} channels")
    ...
    def compute_loss(self, other):
        ...
        if self.width != other.width or self.height != other.height:
            raise ValueError(f"Canvas dimensions must match: ({self.width}, {self.height}) vs ({other.width}, {other.height})")
